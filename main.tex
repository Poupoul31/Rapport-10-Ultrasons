...
\begin{center}
\large\textbf{\sffamily Experiment N$^\circ$G7: Effet Piston}\\%
...
\begin{wrapfigure}{r}{0.4\textwidth} % r = right, 35% de la largeur du texte
    \centering
    \includegraphics[width=0.35\textwidth]{img/montage_exp.png}
...
\begin{figure}[h!]
    ...
    \begin{subfigure}[b]{0.48\textwidth}
        \centering
        \includegraphics[width=\textwidth]{img/v_vs_p_fini.png}
        \caption{}
    ...
    \begin{subfigure}[b]{0.48\textwidth}
        \centering
        \includegraphics[width=\textwidth]{img/v_vs_p_inf.png} 
        \caption{}
...
\begin{figure}[h!]
    ...
    \begin{subfigure}[b]{0.48\textwidth}
        \centering
        \includegraphics[width=\textwidth]{img/f_vs_p_fini.png}
        \caption{}
    ...
    \begin{subfigure}[b]{0.48\textwidth}
        \centering
        \includegraphics[width=\textwidth]{img/f_vs_p_inf.png} 
        \caption{}
...
\begin{figure}[h!]
    ...
    \begin{subfigure}[b]{0.48\textwidth}
        \centering
        \includegraphics[width=\textwidth]{img/f_vs_v_fini.png}
        \caption{}
    ...
    \begin{subfigure}[b]{0.48\textwidth}
        \centering
        \includegraphics[width=\textwidth]{img/f_vs_v_inf.png} 
        \caption{}
...
\begin{figure}
    \centering
    \includegraphics[width=0.5\linewidth]{Comparaison_Graphe_Reynolds.png}
...
Le tableau \ref{tab:incertitudes_base} compile les incertitudes absolues estimées sur les mesures directes utilisées lors des calculs.
...
\begin{table}[h]
    ...
    \begin{tabular}{|l|c|}
        ...
        \hline
        Pression ($P$) & $0.5~\mathrm{mbar}$ \\
        Temps de passage capteur ($dt$) & $10^{-4}~\mathrm{s}$ \\
        Longueur du cylindre ($L$) & $1~\mathrm{mm}$ \\
        Distance entre les portes ($\Delta h$) & $1~\mathrm{mm}$ \\
        Masse du cylindre ($m$) & $0.1~\mathrm{g}$ \\
        \hline
    \end{tabular}
    \caption{Incertitudes absolues sur les mesures expérimentales}
    \label{tab:incertitudes_base}
\end{table}

À partir de ces incertitudes de base, les erreurs sur les grandeurs calculées se propagent de la manière suivante :

\paragraph{1. Erreurs sur les vitesses instantanées ($v_i$)}
La vitesse est donnée par $v_i = \frac{L}{dt_i}$. L'incertitude absolue correspondante est obtenue par la somme des incertitudes relatives :
\begin{equation}
    \Delta v_i = v_i \left( \frac{\Delta L}{L} + \frac{\Delta dt}{dt_i} \right)
\end{equation}

\paragraph{2. Erreurs sur le temps de vol inter-portes ($\Delta t_i$)}
Suite au problème technique mentionné dans le rapport, le temps de vol a été approximé par $\Delta t_i = \frac{2 \Delta h_i}{v_i + v_{i-1}}$. En appliquant la propagation des erreurs à ce quotient, on obtient :
\begin{equation}
    \Delta (\Delta t_i) = \Delta t_i \left( \frac{\Delta h}{\Delta h_i} + \frac{\Delta v_i + \Delta v_{i-1}}{v_i + v_{i-1}} \right)
\end{equation}

\paragraph{3. Erreurs sur les accélérations ($a_i$)}
L'accélération moyenne entre deux portes est $a_i = \frac{v_i - v_{i-1}}{\Delta t_i}$. L'erreur absolue sur le numérateur est $\Delta(v_i - v_{i-1}) = \Delta v_i + \Delta v_{i-1}$. L'incertitude sur l'accélération s'écrit donc :
\begin{equation}
    \Delta a_i = |a_i| \left( \frac{\Delta v_i + \Delta v_{i-1}}{|v_i - v_{i-1}|} + \frac{\Delta (\Delta t_i)}{\Delta t_i} \right)
\end{equation}

\paragraph{4. Erreurs sur la force de frottement ($F_i$)}
L'équation du mouvement donne $F_i = m(g - a_i)$. L'accélération de la pesanteur $g$ étant considérée comme une constante sans erreur ($9.81~\mathrm{m/s^2}$), l'incertitude absolue sur la force devient :
\begin{equation}
    \Delta F_i = \Delta m |g - a_i| + m \Delta a_i
\end{equation}
\end{document}
